...
function hasRequestHeader(
  ...
  if (headers.has)
    return headers.has(headerName) || headers.has(headerName.toLowerCase());
...
function withFetchAuthHeaders(
  headersInit?: HeadersInit,
  forceAuthorizationHeader = false,
): Headers {
...
export async function fetchWithApiAuth(
  ...
  const response = await fetch(input, {
    ...init,
    headers: withFetchAuthHeaders(init.headers),
  });
  ...
  try {
    ...
    return fetch(input, {
      ...init,
      headers: withFetchAuthHeaders(init.headers, true),
    });
  } catch (refreshError) {
