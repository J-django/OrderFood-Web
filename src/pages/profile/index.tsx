import { Link } from "react-router";
import { useDocumentTitle } from "@/hooks";

const profileEntries = [
  {
    title: "我的家庭",
    path: "/profile/families",
    icon: "icon-[lucide--house-heart]",
  },
  {
    title: "我的订单",
    path: "/profile/orders",
    icon: "icon-[lucide--receipt-text]",
  },
  {
    title: "添加菜品",
    path: "/profile/dishes/new",
    icon: "icon-[lucide--utensils]",
  },
] as const;

export default function ProfilePage() {
  useDocumentTitle("我的");
  return (
    <div className="min-h-[calc(100dvh-3.5rem)] bg-[#f8f8f8] pt-[calc(env(safe-area-inset-top)+1.25rem)]">
      <section className="px-4 pb-5" aria-label="用户信息">
        <div className="flex items-center gap-3">
          <div className="grid size-16 place-items-center overflow-hidden rounded-full bg-[#fff0e8] text-[#ff5f15]">
            <span className="icon-[lucide--user-round] size-7" />
          </div>
          <div>
            <h1 className="text-lg font-semibold text-[#222]">微信用户</h1>
            <p className="mt-1 text-xs text-[#999]">欢迎回来</p>
          </div>
        </div>
      </section>
      <section
        className="mx-3 overflow-hidden rounded-2xl bg-white py-1"
        aria-label="我的功能"
      >
        {profileEntries.map((entry) => (
          <Link
            key={entry.path}
            to={entry.path}
            className="text-4 flex items-center justify-between border-b border-[#f0f0f0] px-3.5 py-2.5 text-[#222] last:border-b-0"
          >
            <span className="flex items-center gap-3">
              <span className={`${entry.icon} size-5 text-[#ff5f15]`} />
              <span>{entry.title}</span>
            </span>
            <span className="icon-[lucide--chevron-right] size-5 text-[#999]" />
          </Link>
        ))}
      </section>
    </div>
  );
}
